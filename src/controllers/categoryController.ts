...
import {
  decodeCreateCategoryDTO,
  decodeUpdateCategoryDTO,
} from "../services/category/decodeCategoryDTO.js";
import {
  createCategory,
  getAllCategories,
  getCategoryByID,
  updateCategory,
  deleteCategory,
} from "../services/category/categoryCRUD.js";
  ...
  try {
    ...
    if (typeof newCategory.title === "string") {
      ...
      setResponse(response, 400, {
        error: "Cannot possible to creating categpry without a title",
      });
    }
  } catch (error) {
...
export async function updateCategoryWithId(
  request: IncomingMessage,
  response: ServerResponse,
  parameters: Record<string, string>
) {
  try {
    ...
    const updatedCategoryDTO = await decodeJSONBody(
      request,
      decodeUpdateCategoryDTO
    );

    if (typeof updatedCategoryDTO.title === "string") {
      const updatedCategoryResult = await updateCategory(
        token.userID,
        categoryID,
        updatedCategoryDTO.title
      );
...
export async function deleteCategoryWithID(
  request: IncomingMessage,
  response: ServerResponse,
  parameters: Record<string, string>
) {
