...
import { decodeCreateExpenseDTO, decodeExpenseSearch, decodeUpdateExpenseDTO } from "../services/expenses/decodeExpenseDTO.js";
import { createNewExpense, deleteExpense, getAllExpenses, getAllExpensesByCategory, getExpensesByDate, getExpenseByID, updateExpense } from "../services/expenses/expenseCRUD.js";
...
export async function createNewExpenses(request: IncomingMessage, response: ServerResponse) {
    try {
...
export async function getExpenses(request: IncomingMessage, response: ServerResponse, parameters: Record<string, string>) {
    try {
        const token = await getAccessTokenValue(request);
...
export async function getExpensesByCategory(request: IncomingMessage, response: ServerResponse, parameters: Record<string, string>) {
    try {
        const token = await getAccessTokenValue(request)
        ...
        const expenses = await getAllExpensesByCategory(token.userID, utilParameters.category, page);
        setResponse(response, 200, expenses);
    } catch (error) {
...
export async function getAllExpensesByDate(request: IncomingMessage, response: ServerResponse, parameters: Record<string, string>) {
    try {
        const token = await getAccessTokenValue(request)
        const queryDate = await decodeJSONBody(request, decodeExpenseSearch);
        const pageParameter = (parameters as { page: string }).page;
        const page = pageParameter ? parseInt(pageParameter, 10) : 1;
        ...
        const expenses = await getExpensesByDate(token.userID, queryDate.initialDate, queryDate.finalDate, page);
        setResponse(response, 200, expenses);
    } catch (error) {
...
export async function getExpense(request: IncomingMessage, response: ServerResponse, parameters: Record<string, string>) {
    try {
        const token = await getAccessTokenValue(request)
...
export async function updateCurrentExpense(request: IncomingMessage, response: ServerResponse, parameters: Record<string, string>) {
    try {
        const token = await getAccessTokenValue(request)
        const expenseID = (parameters as { id: string }).id;
        ...
        const updatedExpense = await decodeJSONBody(request, decodeUpdateExpenseDTO);
        const expense = await updateExpense(updatedExpense, expenseID, token.userID);
        setResponse(response, 200, expense);
    } catch (error) {
...
export async function deleteCurrentExpense(request: IncomingMessage, response: ServerResponse, parameters: Record<string, string>) {
    try {
        const token = await getAccessTokenValue(request)
        const expenseID = (parameters as { id: string }).id;
