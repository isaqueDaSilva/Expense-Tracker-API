...
import {
  ...
  getJWTValue,
  RefreshToken,
} from "../services/jwt/jwtService.js";
...
import {
  ...
  deleteSession,
  updateSession,
} from "../services/sessions/sessionCRUD.js";
import { setResponse } from "../services/setResponse.js";
import { database } from "../app.js";
import {
  getAccessTokenValue,
  verifyRefreshTokenValidity,
} from "../services/jwt/getTokens.js";
import { verifyAccessToken } from "../services/jwt/verifyToken.js";
...
export async function signup(
  ...
  try {
    ...
    await createSession(
      createdUser.id,
      pairOfTokens.accessToken.verificationCode,
      pairOfTokens.refreshToken.token,
      pairOfTokens.refreshToken.id
    );
...
export async function performCleanupOfExpiredSessions(
  request: IncomingMessage,
  response: ServerResponse
) {
  try {
    ...
    if (accessTokenValue === process.env.CLEANUP_JWT_SECRET) {
      const deletedSessionsCount = await deleteAllExpiredSessions();
      setResponse(response, 200, {
        message: `Expired sessions cleaned up successfully. Total deleted: ${deletedSessionsCount}`,
      });
      return;
    } else {
      setResponse(response, 403, { error: "Forbidden." });
      return;
    }
  } catch (error) {
