...
export async function verifyAccessToken(token: string, secret: string | undefined): Promise<string> {
    try {
        ...
        if (typeof verificationCode === "string") {
            ...
            throw new Error("The fields user id and verification code seams that is not contained at the token payload.");
        }
    } catch (error) {
        throw error;
    }
...
export async function isRefreshTokenValid(refreshTokenID: string, token: string, secret: string | undefined) {
    try {
