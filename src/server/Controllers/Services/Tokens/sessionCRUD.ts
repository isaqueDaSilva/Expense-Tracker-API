...
export function createSession(userID: string, accessTokenVerificationCode: string, refreshToken: string, refreshTokenID: string): NeonQueryPromise<false, false, Record<string, any>[]> {
    ...
    const values = [userID, accessTokenVerificationCode, refreshTokenID, refreshToken];
    return database.query(newSession, values);
};

export async function isValidationCodeExist(userID: string, verificationCode: string): Promise<boolean> {
    const session = `
    ...
    if (result.length == 1) {
        ...
        if (sessionCount == '1') {
            ...
            console.log(`Exist more than 1 session with the same verification code. Look at the database soon as possible. Count ${sessionCount}`);
...
export async function getRefreshToken(refreshTokenID: string): Promise<string> {
    ...
    if (result.length == 1) {
        ...
        if (typeof refreshToken === "string") {
            ...
            console.log(`Seams that the refresh token with id ${refreshTokenID}, not exists at the database or the type is not a string.`);
...
export async function updateSession(currentRefreshTokenID: string, newRefreshTokenID: string, newRefreshToken: string, newVerificationCode: string ) {
    const updateSession = `
        UPDATE sessions
        SET at_verification_code = $1, refresh_token_id = $2, refresh_token = $3, updated_at = CURRENT_DATE
        WHERE refresh_token_id = $4;
    `;

    const values = [newVerificationCode, newRefreshTokenID, newRefreshToken, currentRefreshTokenID];
    await database.query(updateSession, values);
};

export function deleteSession(refreshTokenID: string): NeonQueryPromise<false, false, Record<string, any>[]>  {
     const deleteSession = `
