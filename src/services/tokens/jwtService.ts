...
function createRefreshToken(userID: string, issuedAt: Date): RefreshToken {
    ...
    return { id: id, token: refreshToken, expiresOn: refreshTokenExpirationDate.toISOString() }
};

export function createJWT(userID: string): { accessToken: AccessToken, refreshToken: RefreshToken } {
...
export function verifyJWT(token: string, secret: string | undefined, isRefreshToken: boolean): { userID: string, verificationCode?: string, error?: Error} {
    ...
    try {
        ...
        return { userID: decoded.subject, verificationCode: decoded.validationCode };
    } catch (error) {
        throw error;
    }
};
